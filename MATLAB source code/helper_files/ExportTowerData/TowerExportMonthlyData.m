...
if round(nArgs/2)~=nArgs/2
    error('TowerExportMonthlyData.m needs propertyName/propertyValue pairs')
end
...
t0 = datenum(t0);
% now work through
tstop = ceil(datenum(options.to_date));
while t0 < tstop
    ...
    if ~isempty(all_data)
        % save matlab data
        [s, mess, messid] = mkdir(fullfile(options.export_path,'10min','mat'));
        waituntilunlock(fullfile(options.export_path,'10min','mat',...
            [datestr(t0,'yyyy_mmmm') 'mat']))
        lock(fullfile(options.export_path,'10min','mat',...
            [datestr(t0,'yyyy_mmmm') 'mat']),...
            'TowerExportMonthlyData.m')
        save(fullfile(options.export_path,'10min','mat',...
            datestr(t0,'yyyy_mmmm')),...
            'all_data','t0','t1');
        unlock(fullfile(options.export_path,'10min','mat',...
            [datestr(t0,'yyyy_mmmm') 'mat']))
        
        % convert matlab to text and save it
