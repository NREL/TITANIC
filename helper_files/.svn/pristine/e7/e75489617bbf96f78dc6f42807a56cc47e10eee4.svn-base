% function all_data = data_convert_20Hz_to_ASCII(varargin)
%
% find 20 Hz tower data within a certain time-frame and convert it to text.

function [data_path,data_file] = data_convert_20Hz_to_ASCII(varargin)
...
%% -----------------------
% RAW DATA EXPORT (ASCII)
% ------------------------
%data_path and data_file contain all of the file names
for fi =1:numel(data_path)
	try
		% read the data
		raw_data = load(fullfile(data_path{fi},data_file{fi}));
		% export the data as a text file
		if isempty(output_path)
			output_path = data_path{fi};
		end
		if ~exist(output_path,'dir')
			mkdir(output_path)
        end
        SubWriteRawDataASCII(raw_data,...
			data_path{fi},data_file{fi},...
			output_path)
		disp(['Wrote ' data_file{fi} ' to file'])
	end
end