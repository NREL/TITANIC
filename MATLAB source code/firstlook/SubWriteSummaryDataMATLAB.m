	...
	if (di<=numel(datastream))
		if ~isempty(datastream{di})
			...
			if datastream{di}.qc.doqc
				...
				fields = fieldnames(qcd.datastream{di}.data.downsampled.statistics);
				for i =1:numel(fields)
					switch fields{i}
						case{'mean'}
							VAL{end+1} = qcd.datastream{di}.data.downsampled.statistics.(fields{i});
...
%% and write it out
waituntilunlock(fullfile(output_path,output_file))
lock(fullfile(output_path,output_file),'SubWriteSummaryDataMATLAB.m')
save(fullfile(output_path,output_file),'-struct','data_out')
unlock(fullfile(output_path,output_file))